...
public class ArPlastic extends AppCompatActivity {
    private static final String TAG = "ArPlastic";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ar_plastic);
        ...
        if (currentPlaneDetected != planeDetected) {
            ...
            if (planeDetected) {
                if (!isRecycleBinPlaced) {
                    message="Tap the plane to place brown recycle bin";
                    statusTextView.setText(message);
                } else if (!isNewspaperPlaced&& isRecycleBinPlaced) {
                    message="Tap the plane to place plastic bottle";
                    statusTextView.setText(message);
    ...
    private void handleCollision() {
        ...
        isNewspaperThrown = false;
        statusTextView.setText("Great job! Plastic bottle recycled! Tap to place another plastic bottle");
    ...
    private void onPlaneTap(HitResult hitResult, Plane plane, MotionEvent motionEvent) {
        if (!isRecycleBinPlaced) {
            placeRecycleBinModel("models/recycleplastic.glb", hitResult);
        ...
        if (isRecycleBinPlaced && !isNewspaperPlaced) {
            statusTextView.setText("Tap to place another plastic bottle");
        } else if (isRecycleBinPlaced && isNewspaperPlaced) {
            statusTextView.setText("Throw the plastic bottle to the brown recycle bin");
            messageSnackbarHelper.showMessage(this, "Throw the plastic bottle to the brown recycle bin");
        }
    }
    ...
    private void placeNewspaper(HitResult hitResult) {
        ...
        ModelRenderable.builder()
                .setSource(this, Uri.parse("models/plastic_bottle.glb"))
                .setIsFilamentGltf(true)
                .build()
                .thenAccept(renderable -> {
                    AnchorNode anchorNode = new AnchorNode(anchor);
                    ...
                    messageSnackbarHelper.showMessage(this, "Throw the plastic bottle to the brown recycle bin (For recycling paper)");
                })
                .exceptionally(throwable -> {
                    Log.e(TAG, "Unable to load plastic bottle model", throwable);
                    messageSnackbarHelper.showError(this, "Failed to load plastic bottle model: " + throwable.getMessage());
                    return null;
                });
    }
