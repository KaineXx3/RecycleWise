...
public class ArAluminum extends AppCompatActivity {
    private static final String TAG = "ArAluminum";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ar_cans);
        ...
        if (currentPlaneDetected != planeDetected) {
            ...
            if (planeDetected) {
                if (!isRecycleBinPlaced) {
                    message="Tap the plane to place orange recycle bin";
                    statusTextView.setText(message);
                } else if (!isNewspaperPlaced&& isRecycleBinPlaced) {
                    message="Tap the plane to place aluminum cans";
                    statusTextView.setText(message);
    ...
    private void handleCollision() {
        ...
        isNewspaperThrown = false;
        statusTextView.setText("Great job! Aluminum cans recycled! Tap to place another aluminum cans");
    ...
    private void onPlaneTap(HitResult hitResult, Plane plane, MotionEvent motionEvent) {
        if (!isRecycleBinPlaced) {
            placeRecycleBinModel("models/recyclecans.glb", hitResult);
        ...
        if (isRecycleBinPlaced && !isNewspaperPlaced) {
            statusTextView.setText("Tap to place another aluminum cans");
        } else if (isRecycleBinPlaced && isNewspaperPlaced) {
            statusTextView.setText("Throw the aluminum cans to the orange recycle bin");
            messageSnackbarHelper.showMessage(this, "Throw the aluminum cans to the orange recycle bin");
        }
    }
    ...
    private void placeNewspaper(HitResult hitResult) {
        ...
        ModelRenderable.builder()
                .setSource(this, Uri.parse("models/alumium_can.glb"))
                .setIsFilamentGltf(true)
                .build()
                .thenAccept(renderable -> {
                    AnchorNode anchorNode = new AnchorNode(anchor);
                    ...
                    messageSnackbarHelper.showMessage(this, "Throw the aluminum cans to the orange recycle bin (For recycling paper)");
                })
                .exceptionally(throwable -> {
                    Log.e(TAG, "Unable to load newspaper model", throwable);
                    messageSnackbarHelper.showError(this, "Failed to load aluminum cans model: " + throwable.getMessage());
                    return null;
                });
    }
